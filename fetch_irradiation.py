...
for Mountain View Shopping Centre (Gordons Bay) and stores it in data/irradiation_data.json.

Accumulates daily records over time so the dashboard can show
irradiation history alongside generation data.

API: https://api.open-meteo.com/v1/forecast
Location: Mountain View Shopping Centre (Gordons Bay), Saldanha Bay, Western Cape
"""
...
# ── Configuration ──────────────────────────────────────────────────────────
LATITUDE = -34.1553887056237
LONGITUDE = 18.87210565550177
...
def main():
    print("🌤️  Mountain View Shopping Centre (Gordons Bay) - Irradiation Data")
    print("=" * 50)
    ...
    # NOTE: no pruning — irradiation history is retained indefinitely so
    # expected-generation / performance comparisons have a long reference
    # baseline. Each record is ~300 bytes; a decade is ~1 MB.
