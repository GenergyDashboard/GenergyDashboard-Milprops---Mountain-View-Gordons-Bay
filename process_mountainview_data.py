"""
process_mountainview_data.py

Reads today's scraped data (data/mountainview_raw.xlsx) and adds it to the
starting values (data/starting_values.json) to produce an updated
output (data/mountainview_processed.json).
...
Flow:
  ...
  5. Save updated mountainview_processed.json
...
def main():
    data_dir = Path("data")
    raw_file = data_dir / "mountainview_raw.xlsx"
    starting_file = data_dir / "starting_values.json"
    output_file = data_dir / "mountainview_processed.json"
    
    print("🔄 Processing Mountain View Shopping Centre (Gordons Bay) data...")
    ...
    # ── Build output ───────────────────────────────────────────────────────
    output = {
        "plant": "Mountain View Shopping Centre (Gordons Bay)",
    ...
    try:
        ...
        # Seasonal averages: pull every stored day whose MONTH matches today's
        # month, across ALL historical years in the file. This produces a
        # proper seasonal load/grid/PV profile (May behaves like past Mays,
        # not like today's partial data). d is "YYYY-MM-DD" so d[5:7] is MM.
        current_mm = now.strftime("%m")
        month_load_days = {d: hrs for d, hrs in hourly_gen.get("days_load", {}).items()
                          if len(d) >= 7 and d[5:7] == current_mm}
        ...
        # NOTE: no pruning — hourly PV / load / grid arrays are retained
        # indefinitely to build a continuous seasonal profile for expected-value
        # calculations. Upgrade storage if the file starts to strain GitHub's
        # 100 MB/file limit (at ~3 KB/day, that's ~90 years of history).
    ...
    try:
        ...
        # NOTE: no pruning — daily_history grows indefinitely. Each day is
        # ~4-5 KB of JSON; a decade of data is well under 20 MB and GitHub
        # handles it fine. Retain as long as storage permits.
